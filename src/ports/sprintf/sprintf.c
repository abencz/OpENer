/*
 * This library was modified to adapt it for not-an-OS.
 *
 *******************************************************************************
 ...
 *******************************************************************************
 ...
 modified Septemnber 2010 by Jonathan Engdahl jrengdahl@ra.rockwell.com
...
static int printi(char **out, char *bufend, int i, int b, int sg, int width, int pad, int letbase)
{
...
int vsnprintf(char *buf, size_t size, const char *format, va_list args )
{
	...
	for (; *format != 0  && out<bufend; ++format) {
		if (*format == '%') {
			...
			if( *format == 'd' ) {
				pc += printi (&out, bufend, va_arg( args, int ), 10, 1, width, pad, 'a');
				continue;
			}
			if( *format == 'x' ) {
				pc += printi (&out, bufend, va_arg( args, int ), 16, 0, width, pad, 'a');
				continue;
			}
			if( *format == 'X' ) {
				pc += printi (&out, bufend, va_arg( args, int ), 16, 0, width, pad, 'A');
			...
			if( *format == 'u' ) {
				pc += printi (&out, bufend, va_arg( args, int ), 10, 0, width, pad, 'a');
				continue;
			}
			if( *format == 'c' ) {
