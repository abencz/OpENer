 ...
 *
 * SVN $Revision: 2817 $
 * SVN $Date: 2010-07-13 13:48:23 +0100 (Tue, 13 Jul 2010) $
 */
#include "mss_nvm.h"
...
)
{
    ...
    if ( (nvm_block_idx < 0) || (( start_addr + nb_bytes ) > end_addr) )
    {
        /* Return immediatly if address out of range. */
    ...
    else
    {
        ...
        while ((nb_bytes_left > 0u) && (NVM_SUCCESS == outcome))
        {
           ...
           uint32_t page_start_addr;
            ...
            page_start_addr = (uint32_t)p_byte_nvm & PAGE_ALIGN_MASK;
            ...
            else
            {
                 ...
                 */
                nb_bytes_in_page = (page_start_addr + 128u) - ((uint32_t)p_byte_nvm);
