...
void dp_load_row_address(void)
{
    ...
    DPUCHAR Address[24]= { 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 
...
void dp_smart_erase(void)
{
    ...
    global_uchar = 0x5u;
    dp_read_factory_row();
    SE_iteration = global_buf1[14] >> 1u;
    SE_iteration |= (DPUCHAR)(global_buf1[14] << 7u);
    
    for(DataIndex = 0u; DataIndex < SE_iteration; DataIndex++)
    {
        dp_erase_tighten();
        
        opcode = ISC_SMART_ERASE;
        IRSCAN_in();
        DRSCAN_in(0u, 2u, (DPUCHAR*)DPNULL);
        goto_jtag_state(JTAG_RUN_TEST_IDLE,device_se_wait_cycle);
        dp_poll_smart_erase();    
        
        if ( (error_code != DPE_SUCCESS) || ( (global_uchar & 0x2u) == 0x2u))
        {
            if (error_code != DPE_SUCCESS)
            {
                #ifdef ENABLE_DEBUG
...
void dp_erase_tighten(void)
{
    ...
    for ( RowNumber = (DPINT)device_rows - 1; RowNumber >= 0; RowNumber-- )
    {
        ...
        IRSCAN_in();
        DRSCAN_in(0u, 3u, (DPUCHAR*)DPNULL);
        
        goto_jtag_state(JTAG_RUN_TEST_IDLE,ISC_ERASE_TIGHTEN_CYCLES);
...
void dp_poll_erase_tighten(void)
{
    ...
    for ( global_ulong = 0u; global_ulong <= MAX_ERASE_TIGHTEN_POLL; global_ulong++ )
    {
        dp_delay(ERASE_TIGHTEN_POLL_DELAY);
        opcode = ISC_ERASE_TIGHTEN;
        IRSCAN_in();
        DRSCAN_out(3u, (DPUCHAR*)DPNULL, &global_uchar);
        if ((global_uchar & 0x1u) == 0u)
        {
            break;
        }
    }
    if (global_ulong >  MAX_ERASE_TIGHTEN_POLL)
    {
...
void dp_poll_smart_erase(void)
{
    ...
    {
        DRSCAN_out(2u, (DPUCHAR*)DPNULL, &global_uchar);
        
        if ((global_uchar & 0x1u) == 0u)
        {
            break;
        }
        dp_delay(SMART_ERASE_POLL_DELAY);
        opcode = ISC_SMART_ERASE;
        IRSCAN_in();
    }
