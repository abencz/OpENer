...
void dp_program_nvm_block(DPUCHAR BlockNum)
{
    ...
    for (Par=0u ; Par < NumOfPart; Par++)
    {
        ...
        ParSize = dp_get_bytes (NvmParSize_ID + BlockNum*NVM_OFFSET,
        (Par*32u) >> 3u, 4u);
        Protection = (DPUCHAR) dp_get_bytes(NvmProtect_ID + BlockNum*NVM_OFFSET,
        (Par*3u) >> 3u, 1u) << 2u;
        ...
        bit postion 0 */
        global_ulong = dp_get_bytes (NvmAddr_ID + BlockNum*NVM_OFFSET,
        (Par*22u) >> 3u, 
        (DPUCHAR) ((((Par*22u) & 7u) + 22u + 7u) >> 3u)) >> ((Par*22u) & 7u) ;
    ...
    for (Par=0u;Par < NumOfPart; Par++)
    {
        ...
        for (i = 1u; i <= ParSize; i++)
        {
            ...
            if (error_code == DPE_SUCCESS)
            {
                ...
                dp_delay(NVM_PROGRAM_ENC_DELAY);
                if (global_buf1[0] & 0x10u)
                {
                    error_code = DPE_NVM_PROGRAM_ERROR;
                    j = 16u;
