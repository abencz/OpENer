...
DPUCHAR device_se_wait_cycle; /* Holds the value of the smart erase wait time */
...
void dp_check_action(void)
{
    ...
    else if (Action_code == DP_ERASE_ACTION_CODE)
    {
        ...
        if (hardware_interface == IAP_SEL)
        {
            ...
            (dat_support_status & SEC_DAT_SUPPORT_BIT))
            {
                #ifdef ENABLE_DEBUG
                dp_display_text("\r\nFROM and security erase is not supported with IAP. Refer to user guide for more information.");
                #endif
                error_code = DPE_ACTION_NOT_SUPPORTED;
            }
        }
    ...
    else if (Action_code == DP_PROGRAM_ACTION_CODE)
    {
        ...
        if (dat_support_status & FROM_DAT_SUPPORT_BIT)
        {
            ...
            {
                #ifdef ENABLE_DEBUG     
                dp_display_text("\r\nFROM programming is not supported with IAP. Refer to user guide for more information.");
                #endif
                error_code = DPE_ACTION_NOT_SUPPORTED;
            }
            #endif
    ...
    else if ( (Action_code == DP_PROGRAM_NVM_ACTION_CODE) || 
...
void dp_check_action(void)
{
    ...
    #endif
    #ifdef ENABLE_IAP_SUPPORT            
    const DPUINT iap_code_status = CORE_CODE_PLAIN_SUPPORT_BIT | 
    CORE_CODE_ENCRYPTION_BIT | 
    NVM_CODE_PLAIN_SUPPORT_BITS  | 
    NVM_CODE_ENCRYPTION_BITS;
    #endif
    ...
    */
    ...
    if(hardware_interface == GPIO_SEL)
    {
        ...
        * up with the definitions above*/
        dat_status |= (dat_support_status & 0xFD00);
        ...
        if (hardware_interface == IAP_SEL)
        {
            dat_status = dat_status & (dat_status ^ iap_code_status);
        }
        #endif
        ...
        if (dat_status)
        {
            ...
            if (hardware_interface == IAP_SEL)
            {
                dp_display_text("\r\nUser Error: IAP does not support FROM or security programming.  Regenerate data file without FROM and security support.");
            }
            #endif
            dp_display_text("\r\nUser Error: Compiled code does not support the requesed action.  Re-compile the code with the arropriate compile option enabled...");            
            #endif
...
void dp_perform_action (void)
{
    ...
    if (error_code == DPE_SUCCESS)
    {
        ...
        {
            if ((Action_code == DP_PROGRAM_NVM_ACTIVE_ARRAY_CODE) || 
            (Action_code == DP_VERIFY_NVM_ACTIVE_ARRAY_CODE))
            {
...
void dp_check_device_ID(void)
{
    ...
    * Therefore, it can be used to store the Data file ID for */
    ...
    if ( device_ID == DataIndex )
    {
        ...
        device_se_wait_cycle = (DPUCHAR) dp_get_bytes(Header_ID,SE_WAIT_OFFSET,1U);
...
****************************************************************************/
#if ((!defined ENABLE_V85_DAT_SUPPORT) && (!defined ENABLE_CODE_SPACE_OPTIMIZATION))
...
void dp_initialize(void)
{
    ...
    if (error_code == DPE_SUCCESS)
    {
        ...
        {                 
            ...
            /* Perform smart erase and discrete address shifting check if applicable. */
...
void dp_exe_erase(void)
{
    ...
    if (error_code == DPE_SUCCESS)
    {
        if ((device_family & SMART_ERASE_BIT) && (global_buf1[0] & 0x1U))
        {
            dp_smart_erase();
            if (error_code != DPE_SUCCESS)
            {
...
void dp_display_urow(void)
{
    ...
    algo_version = (global_buf2[3] >> 1) & 0xfu;
    if ((global_buf2[0] & 0x1u) != (global_buf2[0] & 0x20u) >> 5u)
    {
        algo_version |= ((global_buf2[2] >> 7u ) & 0x1) << 4u;
        algo_version |= (global_buf2[3] & 0x1u) << 5u;
        algo_version |= ((global_buf2[0] >> 5u) & 0x1u) << 6u;
    }
    programming_method = (global_buf2[3] >> 5u) & 0x7u;
    programmer = ((global_buf2[0] >> 6u) & 0x3u) | ((global_buf2[1] & 0x3u) << 0x2u);
    software_version = (global_buf2[1] >> 2u) | (global_buf2[2] & 0x1 << 6u);
...
void dp_program_urow(void)
{
    ...
    global_buf2[2] |= (DC_SOFTWARE_VERSION >> 6u) & 0x1;
    
    
    opcode = ISC_PROGRAM_UROW;
    IRSCAN_in();
    DRSCAN_in(0,UROW_BIT_LENGTH,global_buf2);
...
void dp_check_security_settings(void)
{
    DPUCHAR dev_enc_status = 0;
    DPUCHAR dat_enc_status = 0;
    ...
    global_uchar = (DPUCHAR) ((dat_support_status & 0x3) | ((dat_support_status >> 1) & 0x3c));
    ...
    if ((global_uchar & 0x1) && (dev_enc_status & 0x1) == 0)
    ...
    if ((global_uchar & 0x2) && (dev_enc_status & 0x2) == 0)
    ...
    if ((global_uchar & 0x3C) && (dev_enc_status & 0x3C) == 0)
...
/****************************************************************************
* Purpose: To detect if the target device is smart erase enabled device and 
* if discrete address shifting is required if applicable.
****************************************************************************/
#if ( (!defined ENABLE_CODE_SPACE_OPTIMIZATION) || (defined ENABLE_DAS_SUPPORT) )
    void dp_das_smart_erase_check(void)
{
    
    global_uchar = 0U;
    dp_read_factory_row();
    if ((global_buf1[6] & 0x70U) == 0x30U)
    {
